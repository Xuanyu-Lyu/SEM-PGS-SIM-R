...
#BASICS
script.directory <- "/Users/matthewkeller/GoogleCloud/DriveDocuments/RESEARCH/GeneEvolve/GeneEvolve-ETFD/NEWEST.GE75_ETFD"     # directory where all GeneEvolve R scripts exist - NO SPACES in here!
output.directory <- "/Users/matthewkeller/GoogleCloud/DriveDocuments/RESEARCH/GeneEvolve/GeneEvolve-ETFD/NEWEST.GE75_ETFD/GE.Output"     # working directory where all GeneEvolve R output will be written - NO SPACES in here!

make.graphics <- "yes"                # do you want to create a full graphical report at end?

#DEMOGRAPHIC DETAILS:
number.generations <- 4      # number of generations to evolve before pedigree data created
start.pop.size <- 500       # breeding population size at start of simulation; should be > 500
...
m <- .2        # the path coefficient from parenting phenotype
...
number.genes <- 4 # number of genes affecting phenotype; Must be > 2 but <= 50. 5+ is fine #CHANGED
...
vars <- c("A", "A.S.cor","A.U.cor","A.by.AGE","A.by.S","A.by.U","AA","AGE","D","F","MZ","S","TW",
          ...
          "R.Alevel.Aslope.sex","am.model","latent.AM","make.graphics",
