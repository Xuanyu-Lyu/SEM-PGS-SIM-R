...
#Then loops through X generations of AM in order to create directional LD between the CVs & X generations of VT to create G-E covariance
#originaly by mck, May, 2017
#Updated Oct 10, 2023

#The major change with this script compared to Simulate.Multivariate.NOLD.AM_May2021-mck9.R is that this only runs a SINGLE iteration (iterations are how many times to rerun the analysis, not the same as generations, which are how many generations for the population to evolve) and only spits out the phenotypic data for each person for the first generation and the final 2 generations


#To start with when learning, just concentrate on running sections 1-3 and 6, and look at the functions being called in source("Simulate.Multivariate.NOLD.AM.FUNCTIONS_May2021-mck7.R"



###########################################
#1 BELOW are some notes from previous versions - just here FYI in case it becomes relevant:
...
#####################################################################
#2 Functions & source functions & libraries
...
#Assortative mating & VT simulation functions
...
#####################################################################
#3 New attempt to run AM and get out the two haplotypes for each parent
#This section sets the parameters of variance components, etc. in the first generation (base) population, before any VT or AM
...
# WILDCARD parameters
pop.size <- 1e3 #maybe something like 2e4, or 20000, when running for real
num.cvs <- 10 #maybe 25
seed <- 1234567
num.gen <- 5 #8 should be sufficient to get to AM equilibrium
...
vg1 <- .60 #trait 1 vg
vg2 <- .30 #trait 2 vg
rg <- .1 #genetic CORRELATION @t0 bw trait 1 and trait 2 for both obs. PGS and latent PGS (assumed to be the same). NOTE: this is NOT the full rg at t0. It is the rg bw PGSs, and the rg bw LGSs. The full rg may be a bit different (Simpson's paradox)
...
#####################################################################
#4 IMPLIED variables - usually nothing below needs to be input by user
#This section just converts the above inputs into matrices in our algebra
#This should not be that relevant to Noemie in the beginning
...
#####################################################################
#5 Get expected parameter values based on our math
#This section is just useful for figuring out if our basic math is correct (or at least internally consistent)
#Again, this shouldn't be too relevant to Noemie in the beginning

gens <- 50 #doesn't have to match num.gens - just a high enough number of generations to get to equilibrium
...
#####################################################################
#6 This section now simulates the actual data in a forward-time simulation
#This section is useful for seeing if actual simulated data agree with our mathematical expectations
	...
	#CVs and their effect sizes
		...
		#This does the actual simulation - see the script Simulate.Multivariate.NOLD.AM.FUNCTIONS_May2021-mck7.R
...
#These will grab only the last generation's data, EXCEPT for the PHEN data. If you want this same info for other generations, change [[num.gen]] to be, e.g., [[1]] for gen 1, [[2]] for gen 2, etc.		
	SUMMARY.last <- AM.DATA$SUMMARY.RES[[num.gen]]
	COVS.last <- AM.DATA$COVARIANCES[[num.gen]]
	PHEN1 <- AM.DATA$HISTORY$PHEN[[1]]
	PHEN.n2l <- AM.DATA$HISTORY$PHEN[[num.gen-1]]
	PHEN.last <- AM.DATA$HISTORY$PHEN[[num.gen]]
...
#Summary - provides all the matrices for each of the generations in a list 
#PHEN1 - this is the phenotypes and aspects of each phenotype for each person in the first generation. 
#PHEN.n2l - this is the phenotypes and aspects of each phenotype for each person in the next to last (n2l) generation. 
#PHEN.last - this is the phenotypes and aspects of each phenotype for each person in the last generation. 

#COVS - the variance/covariance matrix of all the variables. To see these, run:
...
#####################################################################
#7 Compare expected vs. observed parameters
#This section is probably not immediately useful to Noemie. It's pulling in saved images from multiple different runs (with different parameters for each) to check whether our simulated vs. expected matrices agree
